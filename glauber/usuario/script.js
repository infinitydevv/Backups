		$('#login').keyup(function (){
		  ...
		  var url = "verificalogin.php?login=" + valor;
		...
		$('#email').keyup(function (){
		  ...
		  var url = "verificalogin.php?email=" + valor;
		  xmlHttp.open("GET", url, true);
		  xmlHttp.send();
		});
		...
		$('#senhaC').keyup(function(){
			...
			if(senha != senhaC){
				$('#config').remove();
				$('#conf').append('<div class="alert alert-success" id="config" role="alert">As senhas não correspondem, repita a senha corretamente!</div>');
				$('#buttonn').attr("disabled", "disabled");
			}else{
				$('#config').remove();
				$('#buttonn').removeAttr('disabled');
