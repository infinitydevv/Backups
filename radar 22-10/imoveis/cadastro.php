...
//****************************************
	...
	if(isset ($_POST['gravar'])){  
		...
		areaservicoimo,piscinaimo,academiaimo,valorimo,planoimo,inclinadoimo,gramadoimo,calcadoimo,comissaoimo,valorvendaimo,observacao1,observacao2,observacao3,
		medidasimo,foto1,foto2,foto3,foto4,foto5,destaqueimo,empreendimento,nomeempreendimento,numunidades,numandares,numpredios,aptosporandar,esperasplit,sacada,quadraesporte,mcmv,numlotes, idcap",
		...
		'$servico','$piscina','$academia','$valimo','$plano','$inclinado','$Gramado','$Calcado','$comissao','$valvenda','$observacao1','$observacao','$observacao2',
		'$medidas','$arq_name1','$arq_name2','$arq_name3','$arq_name4','$arq_name5','$destaque','$emp','$empreendimento','$numunidades','$numandares','$numpredios','$aptosporandar','$split','$sacada','$quadra','$MCMV','$numlote', $nomecap","idimo='$getId'"); 
	...
	if(isset ($_POST['editar'])){ 	
	 ...
	 if(isset ($_POST['editar'])){ 
		...
		areaservicoimo='$servico',piscinaimo='$piscina',academiaimo='$academia',valorimo='$valimo',planoimo='$plano',inclinadoimo='$inclinado',gramadoimo='$Gramado',calcadoimo='$Calcado',comissaoimo='$comissao',valorvendaimo='$valvenda',observacao1='$observacao1',observacao2='$observacao',observacao3='$observacao2',
...
<body onload="Inibe()">
	<div class='container'>
			...
			<form action="" name="formulario" method="POST" id='form-contato' enctype='multipart/form-data'>
				...
				<fieldset>
					...
					<legend>Dados Gerais </legend>
						...
						<div class="form-group">
							<label for="tipo" >Captador:</label>
							<br>
							<select  class="form-control" name="nomecap" >
							<?php
								$sql="select * from captador";
								$consulta=mysql_query($sql,$conexao);
								while($campo=mysql_fetch_array($consulta)){
								$idcap=$campo['idcap'];
								$nomecap=$campo['nomecap'];
								echo "<option value='$idcap'>$nomecap</option>";
								}
							?>
							</select>	
	  ...
	  <legend>Proprietário </legend>
		<label for="tipo" >Cliente:</label>
							<br>
							<?php 
								$sql="select a.idimo, b.nomecliven, b.idcliven from imovel a inner join clientevendedor b on a.idcliven = b.idcliven where a.idimo =".$getId ;
								$consulta=@mysql_query($sql,$conexao);
								$campo=@mysql_fetch_array($consulta);
								$idc =  $campo['idcliven'];
								$nomec = $campo['nomecliven'];
							?>
	...
	<div class="form-group">
		<label for="comment">Facilidades de Negociação:</label>
		    <textarea class="form-control ckeditor" rows="5" name="observacao1" placeholder="Facilidades de negociação do imóvel"><?php echo $campo['observacao1']?></textarea>
    </div>		
	<div class="form-group">
	    <label for="comment">Observações:</label>
			 <textarea class="form-control ckeditor" rows="5" name="observacao" placeholder="Observações sobre o imóvel"><?php echo $campo['observacao2']?></textarea>
    </div>
		<div class="form-group">
	    <label for="comment">Descrições do Imóvel:</label>
			 <textarea class="form-control ckeditor" rows="5" name="observacao2" placeholder="Características sobre o imóvel"><?php echo $campo['observacao3']?></textarea>
    </div>
	</fieldset>			
