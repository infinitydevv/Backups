...
if (empty($termo)):
	...
	$sql = 'SELECT i.idimo, i.tipoimo, i.bairro, i.cidade, i.valorvendaimo, u.nome, c.nomecap, cli.idcliven, city.idcity, city.nomecity, i.ativo FROM imovel i INNER JOIN usuario u on i.idusu = u.idusu inner join captador c on i.idcap = c.idcap inner join clientevendedor cli on i.idcliven = cli.idcliven inner join cidade city on city.idcity = i.cidade order by i.idimo';		
	...
	$termo1 = (isset($_GET['combo'])) ? $_GET['combo'] : '';
		  ...
		  if($termo1 == 1 ){
			...
			$sql .= ' WHERE city.nomecity LIKE :cidade order by i.idimo';
			$stm = $conexao->prepare($sql);
