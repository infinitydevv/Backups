...
<html lang="pt-br">
  ...
  <body>
      ...
      <div class="modal fade" id="load" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="progress">
          <div class="progress-bar progress-bar-striped progress-bar-animated" id='carregar' role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" style="width: 0%"></div>
        </div>
      </div>
      <script>
                var im = $('#foto').val();
                ...
                crp.result('base64','viewport').then(function(canvas) {
                  $.ajax({
                    ...
                    uploadProgress: function (event, position, total, percentComplete) {
                      $('#carregar').width(percentComplete + '%');
                      $('#carregar').attr('aria-valuenow', percentComplete);
                    },
                    success: function() {
