<?php 
	 class Usuario{
	 	...
	 	public function updateUsuario(){//UPDATE de usuário
	 		$pdo = new Conexao();
	 	 	return $pdo->query("UPDATE tbusuario SET nome_u = :nome, email_u = :email, login_u = :login, senha_u = :senha where id_u = :id", array(":nome"=>$this->getNome_u(), ":email"=>$this->getEmail_u(), ":login"=>$this->getLogin_u(), ":senha"=>$this->getSenha_u(), ":id"=>$this->getId_u()));
	 	}

		public function updateUsuarioSenha(){//UPDATE de usuário da senha
	 		$pdo = new Conexao();
	 	 	return $pdo->query("UPDATE tbusuario SET senha_u = :senha where id_u = :id", array(":senha"=>$this->getSenha_u(), ":id"=>$this->getId_u()));
	 	}
	 	...
	 	public function listUsuarioConto(){
	 		$pdo = new Conexao();
	 	    return $pdo->select("SELECT * FROM tbusuario u inner join tbconto c on u.id_u = c.id_u where u.id_u = :id", array(":id"=>$this->getId_u()));
	 	}
		...
		public function listarUsuarioNome(){//SELECT dos usuários por Nome
	 	...
	 	public function listarUsuarioEmailV(){//SELECT dos usuários por Email
	 		$pdo = new Conexao();
	 		return $pdo->select("SELECT * FROM tbusuario where email_u like :email", array(":email"=>$this->getEmail_u()));
	 	}
