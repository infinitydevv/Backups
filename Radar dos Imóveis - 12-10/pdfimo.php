<?php
	...
	$imoveis = $stm->fetchAll(PDO::FETCH_OBJ);
	 	$dompdf = new DOMPDF();
	 	$html = "<link rel='stylesheet' type='text/css' href='css/pdf.css'><h1 style='text-align:center;'>Radar dos Imóveis</h1>";
	 				foreach ($imoveis as $imovel){
		 				if(!empty($imovel->nomeempreendimento)){
			              $html .= "<span>".$imovel->nomeempreendimento."</span>";
			            }
			            $html .= "
			              <span>".$imovel->tipoimo."</span>
			              <span>".number_format($imovel->valorvendaimo, 2,',','.')."</span>
			              
				              <span>Código: ".$imovel->idimo."</span>
				              <span>".$imovel->cidade."</span>
				              <span>Bairro: ".$imovel->bairro."</span>
